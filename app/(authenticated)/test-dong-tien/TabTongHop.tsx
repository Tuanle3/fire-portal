...
function resolveKeHoach(it: NganSachItem, kmcpPlanned: Record<string, number>) {
  if (it.kmcp && it.kmcp in kmcpPlanned) return { khVal: kmcpPlanned[it.kmcp], khIsAuto: true }
  return { khVal: it.ke_hoach, khIsAuto: false }
}

export function TabTongHop({ data, tonQuySoDu, tonQuyRealtime, tonQuySoDuLoading, kmcpActual, kmcpPlanned, chiThang, tonQuyDetail = [] }: Props) {
  ...
  // Resolve all items with auto values (Thực hiện + Kế hoạch AUTO vay NH)
  const resolved = useMemo(() =>
    items.map(it => {
      const th = resolveThucHien(it, kmcpActual, tonQuyRealtime)
      const kh = resolveKeHoach(it, kmcpPlanned)
      return { ...it, ...th, khVal: kh.khVal, khIsAuto: kh.khIsAuto }
    }),
    [items, kmcpActual, kmcpPlanned, tonQuyRealtime]
  )
  ...
  const groupTotals = useMemo(() => {
    ...
    for (const it of resolved) {
      ...
      g.kh += it.khVal   // dùng khVal (AUTO nếu vay NH, nhập tay nếu không)
      g.th += it.val
    }
    return m
  }, [resolved])
  ...
  const sectionTotals = useMemo(() => {
    ...
    for (const it of resolved) {
      ...
      if (it.nhom === 'B') { B_kh += it.khVal; B_th += it.val }
      if (it.nhom === 'C') { C_kh += it.khVal; C_th += it.val }
    }
          ...
          {resolved.map(it => {
            ...
            return (
              <tr key={it.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                ...
                <td style={TD({ right: true })}>
                  {it.khIsAuto
                    ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>{fmt(it.khVal)}<AutoBadge /></span>
                    : fmt(it.khVal)}
                </td>
                <td style={TD({ right: true })}>
                  {it.val > 0
